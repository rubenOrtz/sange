...
struct AudioFormat{
    ...
    AVChannelLayout ch_layout;  // Cambiado de channel_layout a ch_layout

    AudioFormat(){}

    void reset(){
        // Copia un valor por defecto de AVChannelLayout (por ejemplo, MONO)
		AVChannelLayout default_layout = AV_CHANNEL_LAYOUT_MONO;
		av_channel_layout_copy(&ch_layout, &default_layout);
		fmt = 0;
    }
};
